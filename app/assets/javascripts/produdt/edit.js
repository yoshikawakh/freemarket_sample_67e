...
//   // 既に使われているindexを除外
//   lastIndex2 = $('.js-file_group:last').data('index');
...
$(document).ready(function(){
  ...
  $('.sell-form__image__dropbox').on('change', '.js-file2', function(e) {
    const targetIndex2 = $(this).parent().data('index');
    // ファイルのブラウザ上でのURLを取得する
    const file = e.target.files[0];
    const blobUrl = window.URL.createObjectURL(file);
    ...
    if (img2 = $(`img[data-index="${targetIndex2}"]`)[0]) {
      ...
      $('#previews2').append(buildImg2(targetIndex2, blobUrl));
      // fileIndexの先頭の数字を使ってinputを作る
      $('.sell-form__image__dropbox').append(buildFileField2(fileIndex2[0]));
      fileIndex2.shift();
      // 末尾の数に1足した数を追加する
      fileIndex2.push(fileIndex2[fileIndex2.length - 1] + 1);
    }
